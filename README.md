...
1. **Drop your files** — CSV/XLSX is parsed by pandas running in your browser via [Pyodide](https://pyodide.org) (WebAssembly). File contents never leave your machine.
2. **Describe what you need** — only the column schema (and, optionally, sample rows) is sent to the backend, which asks Claude (on Amazon Bedrock) to generate a Python `transform(inputs, params)` recipe plus a set of adjustable **knobs**.
3. **See the result** — the recipe runs locally in a web worker; you get output tables and native Plotly charts. Tune the knobs and it re-runs instantly. Edit the script directly if you like.
4. **Save & reuse** — recipes are saved and versioned server-side (text only, never your data). Next month, open a recipe and drop your new files — they're matched to the recipe's named slots by schema and it re-runs. You can also download a standalone `.py`:

   ```sh
   python my-recipe.py orders.csv customers.csv -o output.csv
   ```
...
| Package | What |
|---|---|
| [`frontend/`](frontend/) | Vite + React + TypeScript SPA. Pyodide/pandas runtime, Plotly rendering, all UI. |
...
| Piece | Where |
|---|---|
| Pyodide runtime + pandas execution (web worker) | `frontend/src/lib/pyodideWorker.ts` |
| Worker RPC bridge | `frontend/src/lib/pyodide.ts` |
| Plotly rendering (figures built as dicts in Python) | `frontend/src/lib/plot.ts` |
| Backend API client (generate + recipe CRUD + auth) | `frontend/src/lib/api.ts` |
| Recipe `.py` build/parse | `frontend/src/lib/recipe.ts` |
