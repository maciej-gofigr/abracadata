...
| LLM | **Claude on Amazon Bedrock** (`AnthropicBedrockMantle`, Python `anthropic[bedrock]`), **streaming**, model routing | `anthropic.claude-opus-4-8` for generation, `anthropic.claude-haiku-4-5` for trivial calls (Bedrock IDs carry the `anthropic.` prefix); adaptive thinking + `effort`; meter via `response.usage`; **manual** `cache_control` on the system prompt (auto prompt-caching, Batches, and the Files API are not on Bedrock — none are needed here). Keeps inference in-account with IAM auth and aligns dev with the AWS deploy target. Alt if a Bedrock feature gap bites: *Claude Platform on AWS* (Anthropic-operated, full parity, bare model IDs). |
...
| Hosting (AWS target) | **FastAPI** on ECS Fargate or App Runner (container; holds streaming connections); **Next.js** on Amplify Hosting (or CloudFront + S3) | Deploying to AWS keeps app, backend, and Bedrock inference in one account with IAM auth and no cross-provider egress. (Fly.io / Vercel remain drop-in alternatives if we ever split.) |
